...
<nav>
    ...
    <a  href="song.php?do=Add">اضافة عنصر</a>
...
<?php
  ...
  if($do == "control"){
    ...
    $stmt = $con->prepare("SELECT * FROM library WHERE type='Song' ");
      ...
      <div class="col-lg-4 col-sm-12 " style="margin-top:10px; ">
              <div class="card" style="width: 14rem;" >
                  ...
                  <div class="card-body" style="background-color:#e6ffe6;">
                      ...
                      <br>
                      <a href="song.php?do=Edit&std=<?php echo $u['id'];?>" class="btn btn-primary">تعديل</a>
                      <a href="song.php?do=Delete&std=<?php echo $u['id'];?>"class="btn btn-danger">حذف</a>
...
الرابط :<input type="text" name="link"/>
...
$stmt  = $con->prepare("INSERT INTO library (name , link,type ,image  ) VALUES (:zname , :zlink ,:ztype ,:zfolder )");
...
header("location:song.php");
        ...
        if($stmt1->rowCount() > 0){?>
            ...
            <form action="?do=Update" method="POST" enctype="multipart/form-data">
               ...
               الاسم : <input type="text" name ="name" value="<?php echo $rows['name']?>" class="input-group" />
                ...
                الرابط :<input type="text" name="link" value="<?php echo $rows['link']?>" class="input-group" />
    ...
    elseif($do == "Update"){
        ...
        move_uploaded_file($temp,$folder);
            ...
            $stmt2 = $con->prepare("UPDATE library SET name = ? , link =? , type='Song',image=?  WHERE id = ?");


            $stmt2->execute(array($name,$link,$folder,$id));
